...
import 'package:digi_rupay/components/chat_bubble.dart';
import 'package:digi_rupay/components/payment_tile.dart';
import 'package:digi_rupay/services/auth/auth_service.dart';
import 'package:digi_rupay/services/chat/chat_service.dart';
...
class _ChatPageState extends State<ChatPage> {
    ...
    return Scaffold(
      appBar: PreferredSize(
        preferredSize: Size.fromHeight(
          40.0,
        ), // Set the height to 100.0 or any value you prefer
        child: AppBar(
          title: Text(
            widget.recievername
                .toLowerCase()
    ...
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        return ListView(
          ...
          children:
              snapshot.data!.docs
                  .map<Widget>((doc) => _buildMessageTile(doc))
                  .toList(),
        );
      },
    );
  ...
  Widget _buildMessageTile(dynamic doc) {
    ...
    if (chatsender == null) {
      print(
        "Error: Neither 'senderId' nor 'senderID' exists in the document: $data",
      );
      return Container();
    }
        ...
        child:
            messageType == "payment"
                ? PaymentTile(
                  ...
                  receiverName:
                      isSender ? widget.recievername.capitalize : "You",
                  senderName: isSender ? "You" : widget.recievername.capitalize,
                )
                : ChatBubble(Message: data['message'], iscurrentUser: isSender),
      ),
    );
  }
        ...
        children: [
          Expanded(
            child: TextField(
              ...
              keyboardType:
                  _isSendingPayment ? TextInputType.number : TextInputType.text,
              decoration: InputDecoration(
                hintText:
                    _isSendingPayment ? "Enter amount" : "Type a message...",
                prefixIcon:
                    _isSendingPayment
